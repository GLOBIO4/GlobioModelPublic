...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
class GLOBIO_CalcMSAregion_V2(CalculationBase):
  """
  ...
  def calcRegionMSAAreas(self,csvFileName,msaRasterName,regions,regionRaster,areaRaster):
    ...
    noDataValue = -999.0
    areaRaster_cor = Raster()
    areaRaster_cor.initRaster(self.extent,self.cellSize,np.float64,noDataValue,0)
  ...
  def run(self,*args):
    ...
    self.checkFile(outRegionMSAAreasFileName,asOutput=True)
