...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
class GLOBIO_CalcAquaticLakeDeepMSA(CalculationBase):
  """
  Creates a raster with the aquatic MSA for deep lakes.
  """
  ...
  def run(self,*args):
    ...
    lakeDeepRaster = self.readAndPrepareInRaster(extent,cellSize,lakeDeepRasterName,"deep lake fractions")

    # Create the mask. Select fraction > 0.0.
    Log.info("Selecting deep lake fractions...")
    fracMask = (lakeDeepRaster.r > 0.0)

    # Close and free the fractions raster.
    lakeDeepRaster.close()
    lakeDeepRaster = None
    ...
    # Calculate.
    #
    # N_MSA = exp(a + b * log(NCONC+c))/(1.0 + exp(a + b * log(NCONC+d)))
    #
    ...
    # Initialize with NoData.
    Log.info("Creating MSA for deep lakes raster...")
    noDataValue = -999.0
    outRaster = Raster(outRasterName)
    outRaster.initRaster(extent,cellSize,np.float32,noDataValue)

    #-----------------------------------------------------------------------------
    # Calculate the final MSA for deep lakes.
    #-----------------------------------------------------------------------------

    Log.info("- Calculating MSA for deep lakes...")
    ...
    # Save the MSA raster for deep lakes.
    Log.info("Writing MSA for deep lakes raster...")
    outRaster.write()
    if self.isValueSet(outRasterNameN):
      nMSA.write()
    if self.isValueSet(outRasterNameP):
...
if __name__ == "__main__":
  try:
    ...
    pCalc = GLOBIO_CalcAquaticLakeDeepMSA()
    ...
    frac = os.path.join(inDir,"deep_lake_fractions.tif")
    N_A = 0.145
    N_B = -4.768
    N_C = 0.01
    P_A = -4.002
    P_B = -1.176
    P_C = 0.001
    out = os.path.join(outDir,"deep_lake_msa.tif")
