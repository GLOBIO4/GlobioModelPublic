...
class GLOBIO_CalcAquaticLakeReservoirCyanoBacteria(CalculationBase):
  ...
  def run(self,*args):
    ...
    TemperatureThreshold = args[9]
    NP_threshold = args[10]
    FTNTP_FACTOR_A = args[11]
    FT_FACTOR_A = args[12]
    FT_FACTOR_B = args[13]
    FT_FACTOR_C = args[14]
    FT_FACTOR_D = args[15]
    ...
    self.checkFloat(TemperatureThreshold,-100.0,100.0)
    ...
    self.checkFloat(NP_threshold,-100.0,9999.0)
    ...
    # Select water temperature >= TemperatureThreshold.
    dataMask = np.logical_and(wtDataMask,(waterTempRaster.r >= TemperatureThreshold))

    # Calculate.
    # FT = a + b  * (power(T / TemperatureThreshold,c) - 1.0) 
    ftRaster.r[dataMask] = waterTempRaster.r[dataMask] / TemperatureThreshold
    ...
    # Calculate.
    # FT = d + e  * (power(T / TemperatureThreshold,f) - 1.0) 
    ftRaster.r[dataMask] = waterTempRaster.r[dataMask] / TemperatureThreshold
    ...
    # Calculate.
    # FTNTP = 1.0 - a * (TN / (TP * NP_threshold) - 1.0)
    ftntpRaster.r[ftntpMask] = concNRaster.r[ftntpMask]
    ftntpRaster.r[ftntpMask] /= (concPRaster.r[ftntpMask] * NP_threshold)
