...
#           - Version 4.1.0
#           - Commented out because of serious PyLint error, call 
#             to calcDistance() does not match arguments.
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
class GLOBIO_CalcSettlementDistance_V2(CalculationBase):
  """
  ...
  def __init__(self):
    ...
    super(GLOBIO_CalcSettlementDistance_V2,self).__init__()
  ...
  def calcDistance(self,extent,cellSize,
                   ...
                   landcoverRasterName,landcoverCodesStr,landcoverBufferKM,maskGlobalLuExlCodeStr,
                   outDir,SettlementRasterName):
    ...
    maskGlobalLuExlCodes = self.splitIntegerList(maskGlobalLuExlCodeStr)
    ...
    tmpMaskRasterName = os.path.join(self.outDir,"tmp_mask_humenc.tif")
    ...
    if RU.rasterExists(tmpMaskRasterName):
      RU.rasterDelete(tmpMaskRasterName)

    #-----------------------------------------------------------------------------
    # If needed, read and merge settlement shapefiles.
    #-----------------------------------------------------------------------------
    if self.isValueSet(SettlementRasterName):
      Log.info("Using the provided settlement raster...")
      inputSettlementRaster = self.readAndPrepareInRaster(extent,cellSize,SettlementRasterName,"global settlement")
      # Save the 
      inputSettlementRaster.writeAs(tmpSettlementsRasterName)
      # Close and free the raster.
      inputSettlementRaster.close()
      inputSettlementRaster = None
    else:
    ...
    # JH: check the data type
    Log.info("readAndPrepareInRaster - DataType: %s" % RU.dataTypeNumpyToString(landcoverRaster.dataType))
    ...
    landcoverSelRaster.initRaster(extent,cellSize,np.float32,noDataValue)
    
    # Select specified landcover types (selected = 1).
    for landcoverCode in landcoverCodes:
      #Log.info(landcoverCode)
    ...
    Log.info("Creating mask for land area and tropical biome...")
    # Also do the water mask for the distance analysis below
    # Create empty raster.
    noDataValue = -999
    waterMaskRasterTmp = Raster()
    waterMaskRasterTmp.initRaster(extent,cellSize,np.int32,noDataValue)
    waterMaskRaster = Raster()
    waterMaskRaster.initRaster(extent,cellSize,np.int32,noDataValue)

    # Reads the landcover raster and resizes to extent and resamples to cellsize.
    tropicalBiomeRaster = self.readAndPrepareInRaster(extent,cellSize,tropBiomeRasterName,"tropical biome mask")

    # Set ESA LU water class area as mask, to do: make input
    for maskLuCode in maskGlobalLuExlCodes:
      maskWaterTmp = (landcoverRaster.r == maskLuCode)
      waterMaskRasterTmp.r[maskWaterTmp] = 1
    # set the final mask
    maskWater = ((maskWaterTmp != 1) & (tropicalBiomeRaster.r == 1))
    waterMaskRaster.r[maskWater] = 1
    waterMaskRaster.writeAs(tmpMaskRasterName)
    ...
    waterMaskRaster.close()
    waterMaskRaster = None
    waterMaskRasterTmp.close()
    waterMaskRasterTmp = None
    tropicalBiomeRaster.close()
    tropicalBiomeRaster = None
    ...
    # Filter by mask for settlements outside the land and biome mask (created above) - due to global extent run error
    # Reads the landcover raster and resizes to extent and resamples to cellsize.
    landBiomeMaskRaster = self.readAndPrepareInRaster(extent,cellSize,tmpMaskRasterName,"custom mask")    
    maskLandBiome = (landBiomeMaskRaster.r == landBiomeMaskRaster.noDataValue)
    settlementsRaster.r[maskLandBiome] = noDataValue
    # Cleanup mask
    maskLandBiome =None
    landBiomeMaskRaster.close()
    landBiomeMaskRaster = None
    ...
    gr = Grass()
    gr.init()
    gr.distance(extent,cellSize,tmpSelSettlementsRasterName,tmpDistanceRasterName,tmpMaskRasterName)
    #gr.distance(extent,cellSize,tmpSelSettlementsRasterName,tmpDistanceRasterName)
    gr = None
